...
`git clone https://github.com/ByteLegendQuest/java-create-overload-constructors`

A project directory named `java-create-overload-constructors` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-create-overload-constructors` directory which is created just now.
